...
- `creative/fig-bloom-charcoal-photography` (`fig-bloom-charcoal-photography`): Run charcoal-look AI product photoshoots for Fig & Bloom catalogue products — PDP sets, campaign sets, and single-format shots generated from real Shopify product photos via Higgsfield nano_banana_2, with a full art-direction framework (anchor library, prompt system, failure modes).
