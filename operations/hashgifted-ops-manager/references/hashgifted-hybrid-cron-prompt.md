...
3. Interpret every creator's full ordered conversation yourself. Write one complete strict JSON decision per candidate to the decisions path. Follow the classifier instructions exactly. Conversational meaning is your responsibility; do not classify from keywords, substrings or only the final bubble. For new selection threads, generic portfolio/application text saying someone takes photographs is not a campaign-specific agreement to create and supply the requested photo set. Preserve prior explicit Reel or approved-alternative agreements in existing in-flight threads; do not retroactively change them. Preserve earlier explicit answers when later bubbles add concept detail. Preserve and prioritise pending Daniel-authored Trello commands by source comment ID.
