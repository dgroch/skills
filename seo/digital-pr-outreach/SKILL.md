...
- **Notion `parent.type` field required.** When calling `POST /v1/databases`, the `parent` object MUST include `"type": "page_id"` alongside `"page_id": "..."`. If you only pass `{"page_id": "..."}` without the explicit `type` field, the API returns HTTP 400: `body.parent.type should be defined, instead was 'undefined'`. The `notion` skill's reference doc has been updated to include this.
- **Notion `/markdown` PATCH endpoint does not work with API 2025-09-03.** `PATCH /v1/pages/{page_id}/markdown` returns HTTP 400: `body.type should be defined, instead was 'undefined'`. Use the blocks API instead: `PATCH /v1/blocks/{page_id}/children` with a `children` array of block objects (heading_2, paragraph, bulleted_list_item, callout, divider, etc.). This is the reliable way to write rich content into a Notion page body. See the `notion` skill's `references/block-types.md` for the full block schema.